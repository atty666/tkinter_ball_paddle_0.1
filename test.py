...
class Ball:
    def __init__(self, canvas, rocket, score):
        ...
        self.colors = 'yellow'
        self.color_index = 0
        self.id = canvas.create_oval(10, 10, 35, 35, fill=self.colors)
...
def quit_button():
    quit_game_button = Button(canvas, text='Quit', command=quit_game)
    quit_game_button.place(x=200, y=350)


def restart_game():
    global score, rocket, ball, game_over_text
...
def restart_button():
    restart_game_button = Button(canvas, text='Restart', command=restart_game)
...
restart_button()  # Add restart button initially

tk.bind("<KeyPress-r>", lambda event: restart_game())  # Bind "r" key to restart_game function
